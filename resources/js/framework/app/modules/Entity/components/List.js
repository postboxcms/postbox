...
const List = (props) => {
  ...
  const { convertToFormData } = useData();

  const noRowsMessage = 'No ' + (title ? title : name) + ' added yet';
  ...
  const ImageCell = (params) => {
    ...
    return (
      (!value && <FontAwesomeIcon icon={'image'} size="lg" />) || (
        <img src={`/uploads/${module}/${value}`} className="cell-image" />
      )
    );
  };
        ...
        if (dataValues[index].type == 'image') {
          columnData.map((column) => {
            if (column['field'] == parameter) {
              ...
              columnData.splice(
                columnData.findIndex((col) => col.field === parameter),
                1
              );
            }
          });
                ...
                renderCell: (params) => (
                  <>
                    <FormControlLabel
                      ...
                      control={<IOSSwitch sx={{ m: 1 }} checked={Boolean(params?.value)} />}
                      label=""
                    />
                  </>
                ),
  ...
  return (
    <React.Fragment>
      <div className={classes.header}>
        <Title icon={Icon}>{title ? title : name}</Title>
        <ClassicButton
          onClick={() => addContent()}
          icon="fa-plus"
